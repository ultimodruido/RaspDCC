...
// simy wave of 1ms (1000us = 0h03e8)
// if timer 1 tick = 1us
#define DCC_PAUSE_MSB 0x03
#define DCC_PAUSE_LSB 0xe8

// set max number of buffers for I2C (max 32)
...
int main(void) {
    ...
    // TODO this instruction can be removed
    ...
    while (1) {
        ...
        if (PUSH_BUTTON_EMERGENCY) {
            ...
            // after a package is sent send at least a pause wave, so the PIC
            // can calculate the next xorbuffer
            // I do not trust the assembly code generated by the XC8 free
            // to be fast enought to be generated within the 58us of a HI wave
            DCC_send_bit(DCC_PAUSE_MSB, DCC_PAUSE_LSB);
